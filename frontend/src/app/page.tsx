...
export default function Home() {
  ...
  const getLast7DaysData = (meals: MealInfo[]) => {
    ...
    const last7Days = Array.from({ length: 7 }, (_, i) => {
      ...
      const eatenOut = mealsOnDate.filter(meal => meal.eatingOut).length;
  ...
  const deleteAllMeals = async () => {
    try {
      await axios.delete("https://fzyeqnxwpg.execute-api.us-east-1.amazonaws.com/prod/meals");
      setMeals([]);
      toast({
    ...
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col gap-8 row-start-2 items-center sm:items-start">
        ...
        {isLoading ? (
          ...
          <div className="w-full overflow-x-auto">
            <Table>
              ...
              <TableFooter>
                <TableRow>
                  <TableCell colSpan={4}>Total Meals Tracked</TableCell>
                  <TableCell className="text-right">{meals.length}</TableCell>
                </TableRow>
              </TableFooter>
            </Table>
          </div>
          ...
          <div className="min-h-[200px] w-full">
            ...
            <ChartContainer config={chartConfig} className="min-h-[200px] w-full">
              <BarChart data={chartData}>
                ...
                <Bar dataKey="eatenOut" stackId="a" fill="#0284c7" radius={4}/>
                <Bar dataKey="notEatenOut" stackId="a" fill="#059669" radius={4}/>
              </BarChart>
            </ChartContainer>
          </div>
        )}
        <div className="flex gap-4">
          ...
          <Button onClick={() => setChartView("currentMonth")}>Current Month</Button>
          <Button onClick={() => setChartView("allTime")}>All Time</Button>
        </div>
        <Separator />
        <div className="flex gap-4 items-center flex-col sm:flex-row">
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="destructive">Delete All Meals</Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>
                ...
                <AlertDialogDescription>
                  This action cannot be undone. This will permanently delete all your meals from our servers.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
